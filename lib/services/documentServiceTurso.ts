import { tursoQueries, initializeDatabase } from '../db/turso';
...
export class DocumentService {
  ...
  static async getDocument(id: string): Promise<Document | null> {
    await ensureInitialized();
    const row: any = await tursoQueries.getDocument(id);
    if (!row) return null;
  ...
  static async searchDocuments(query: string): Promise<Document[]> {
    await ensureInitialized();
    const rows: any[] = await tursoQueries.searchDocuments(query);
    return rows.map((row) => ({
      ...row,
      isExpanded: Boolean(row.isExpanded),
    }));
