 ...
 * @category Topics
 */
export function splitSentences(inputText: string, options: {
  ...
  return finalResult
    ...
    .flat()
    .filter(Boolean) as string[]; // Remove null entries from combining short sentences
}
...
function isComplexAbbreviation(word: string): boolean {
  ...
  return matches ? matches[0].length > 0 : false;
}
